...
character_y_pos = screen_height - character_height #화며 세로 가장 아래에 해당하는 곳에 위치
...
enemy_y_pos = (screen_height/2) - (enemy_height/2) #화면 세로 가장 아래에 해당하는 곳에 위치
...
while running:
    dt = clock.tick(30) #게임화면의 초당 프레임 수를 설정 (프레임 수가 낮을수록 캐릭터 이동 속도가 느려짐)
